...
class Utils:
    ...
    def allCallbacks(networkName,currentFold, rater):
        ...
        pathWeights=f"/home/xurbano/QEI-ASL/results/{networkName}/{rater}/{currentFold}"
    ...
    def save_training_plots(history, file_path):
        ...
        MSE = history.history['accuracy'][10:-1]
        val_MSE = history.history['val_accuracy'][10:-1]
        ...
        plt.subplot(1, 2, 2)
        plt.plot(range(1, epochs + 1), MSE, label='Training Accuracy')
        plt.plot(range(1, epochs + 1), val_MSE, label='Validation Accuracy')
        plt.xlabel('Epochs')
        plt.ylabel('MSE')
