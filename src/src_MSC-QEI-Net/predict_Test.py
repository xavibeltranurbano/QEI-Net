...
def scatterPlot(ratings, predictions, currentFold, mse):
    ...
    os.makedirs(f"/home/xurbano/QEI-ASL/results/MSC-QEI-Net/plotsValidation/ScatterPlot",
                exist_ok=True)
    ...
    save_path = f'/home/xurbano/QEI-ASL/results/MSC-QEI-Net/plotsValidation/ScatterPlot/{currentFold}.png'
...
def save_to_excel(all_results):
    ...
    excel_path = f'/home/xurbano/QEI-ASL/results/MSC-QEI-Net/All_Folds_Predictions.xlsx'
        ...
        for fold, data in all_results.items():
            ...
            if 'MSE' in df.columns:  # Check if the 'MSE' column exists
                mean_mse = pd.DataFrame(
                    {'ID': ['Average MSE'], 'Rating': [None], 'Final Pred': [None], 'MSE': [df['MSE'].mean()]})
    ...
    with tf.device('/GPU:0'):
        for currentFold in range(1, 2):
            ...
            for rater in raters:
                ...
                model.load_weights(
                    f"/home/xurbano/QEI-ASL/results/MSC-QEI-Net/{rater}/{currentFold}/Best_Model.keras")
