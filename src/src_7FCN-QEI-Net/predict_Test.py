...
def predict_test():
    ...
    allRatings = []
    allMSE = []
    features = pd.read_excel("/home/xurbano/QEI-ASL/7FCN-QEI-Net/computed_features.xlsx")

    # Initialize the ExcelWriter for the single Excel file
    excel_path = f'/home/xurbano/QEI-ASL/results/7FCN-QEI-Net/All_Folds_Predictions.xlsx'
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        with tf.device('/GPU:0'):
            for currentFold in range(1, 2):
                ...
                # Save fold results to Excel
                save_to_excel(fold_results, '7FCN-QEI-Net', currentFold, writer)

                # Plot the results
                scatterPlot(ratings, predictions, currentFold, np.mean(MSE_vec))

        print(f"Prediction and MSE results saved for each fold in the '7FCN-QEI-Net' directory.")
