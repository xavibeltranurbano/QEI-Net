...
def scatterPlot(ratings, predictions, currentFold, mse):
    os.makedirs(f"/home/xurbano/QEI-ASL/results/QEI-NET_8_features/plotsValidation/ScatterPlot", exist_ok=True)
    ...
    save_path = f'/home/xurbano/QEI-ASL/results/QEI-NET_8_features/plotsValidation/ScatterPlot/{currentFold}.png'
...
def predict_test():
    ...
    allRatings = []
    allMSE = []
    features = pd.read_excel("/home/xurbano/QEI-ASL/src_8_features/computed_features.xlsx")
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        with tf.device('/GPU:0'):
            for currentFold in range(1, 2):
                ...
                model.load_weights(f"/home/xurbano/QEI-ASL/results/QEI-NET_8_features/{currentFold}/Best_Model.keras")
                ...
                for ID in valIDs:
                    ...
                    prediction_tensor = tf.convert_to_tensor([prediction[0]], dtype=tf.float32)  # Ensure prediction is a tensor
                ...
                save_to_excel(fold_results, 'QEI-NET_3_features', currentFold, writer)
                
                # Plot the results
                scatterPlot(ratings, predictions, currentFold, np.mean(MSE_vec))
                
        print(f"Prediction and MSE results saved for each fold in the 'QEI-NET_8_features' directory.")
